...
if __name__ == "__main__":
    ...
    # 加载测试数据
    for i in range(5):
        test_data_file = f"./mmlu_dataset/PG_Benchmark/sampled_mmlu_test_data_user_{i}_prompt.json"
        with open(test_data_file) as test_file:
            test_data = json.load(test_file)

        for ii in range(len(test_data)):
            print(f"user {i}: {ii}th test item")
        ...
        output_file = f"./mmlu_dataset/PG_Benchmark/sampled_mmlu_test_data_user_{i}_results.json"
